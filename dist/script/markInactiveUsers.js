...
const prisma = new PrismaClient();
export const pubsub = new PubSub();
export const markInactiveUsers = new CronJob('* * * * *', // every minute
async () => {
    const now = new Date();
    try {
        ...
        const inactive = await prisma.user.updateMany({
            where: {
                lastUpdatedAt: { lt: new Date(now.getTime() - 15 * 60000) },
                status: { not: 'inactive' }
            },
            data: { status: 'inactive' }
        });
        ...
        const busy = await prisma.user.updateMany({
            where: {
                lastUpdatedAt: { gte: new Date(now.getTime() - 15 * 60000) }, // recent update
                deliveries: {
        ...
        const available = await prisma.user.updateMany({
            where: {
                lastUpdatedAt: { gte: new Date(now.getTime() - 15 * 60000) }, // recent update
                deliveries: {
                    none: { deliveryStatus: { in: ['Pending', 'in_transit', 'assigned'] } }
                },
                status: { not: 'available' }
            },
            data: { status: 'available' }
        });
        console.log(`✅ Updated statuses — Inactive: ${inactive.count}, Busy: ${busy.count}, Available: ${available.count}`);
        // Publish updates so clients know about the status change
